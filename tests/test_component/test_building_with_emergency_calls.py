...
class TestBuildingWithEmergencyCalls():
    ...
    @pytest.fixture
    def COMPONENT_PARAMETERS_BUILDING(self):
        COMPONENT_PARAMETERS_BUILDING = copy.deepcopy(COMPONENT_PARAMETERS)
        COMPONENT_PARAMETERS_BUILDING['ComponentClass'] = 'BuidlingStockUnitWithEmergencyCalls'
        COMPONENT_PARAMETERS_BUILDING['OperationDemand']['Communication'] = {'Amount': 1.0, 
                                                                    'FunctionalityToAmountRelation': 'Linear',
                                                                    'PostDisasterIncreaseDueToEmergencyCalls': 'True'}
        return COMPONENT_PARAMETERS_BUILDING

    def test_update_communication_demand(self, component, COMPONENT_PARAMETERS_BUILDING):
        component.construct(COMPONENT_NAME, COMPONENT_PARAMETERS_BUILDING)
    ...
    def test_modify_emergency_calls(self, component, COMPONENT_PARAMETERS_BUILDING):
        component.construct(COMPONENT_NAME, COMPONENT_PARAMETERS_BUILDING)
        initial_communication_demand = component.demand['OperationDemand']['Communication'].current_amount
    ...
    def test_check_if_demand_increase_considered(self, component, COMPONENT_PARAMETERS_BUILDING):
        component.check_if_demand_increase_considered('DemandResource1', COMPONENT_PARAMETERS_BUILDING['OperationDemand']['DemandResource1'])
        assert component.COMMUNICATION_RESOURCE_NAME == ''

        component.check_if_demand_increase_considered('SupplyResource1', COMPONENT_PARAMETERS_BUILDING['Supply']['SupplyResource1'])
        assert component.COMMUNICATION_RESOURCE_NAME == ''

        component.check_if_demand_increase_considered('Communication', COMPONENT_PARAMETERS_BUILDING['OperationDemand']['Communication'])
